...
Synthea Studio is a React/Vite client for the local FastAPI backend. Cross-runtime domain types come from `contracts/typescript`; all HTTP calls go through `src/api/client.ts` and shared transport/error handling in `src/api/http.ts`.
