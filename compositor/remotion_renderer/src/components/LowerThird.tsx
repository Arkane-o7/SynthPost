...
export const LowerThird: React.FC<{
  ...
  const sourceItems: HeadlineItem[] = headlineItems && headlineItems.length ? headlineItems : [{text: headline}];
  ...
  const currentHeadlineSize = currentHeadline.length > 92 ? 36 : currentHeadline.length > 62 ? 42 : 50;
  const nextHeadlineSize = nextHeadline.length > 92 ? 36 : nextHeadline.length > 62 ? 42 : 50;
  const previousHeadlineSize = previousHeadline.length > 92 ? 36 : previousHeadline.length > 62 ? 42 : 50;
  const activeCueStartFrame = Math.round((orderedCues[activeIndex]?.start ?? 0) * fps);
  const timedTransitionProgress = frame - activeCueStartFrame;
  const isTimedTransition = timedMode && activeIndex > 0 && timedTransitionProgress >= 0 && timedTransitionProgress < transitionFrames;
  const currentOpacity = timedMode
    ? isTimedTransition
      ? interpolate(timedTransitionProgress, [transitionFrames * 0.36, transitionFrames], [0, 1], {
    ...
    : (() => {
        ...
        return orderedCues.length > 1 && slideFrame >= transitionStart
          ? interpolate(slideFrame - transitionStart, [0, transitionFrames * 0.42], [1, 0], {
              extrapolateLeft: 'clamp',
              extrapolateRight: 'clamp',
            })
          : 1;
  ...
  const currentY = timedMode
    ? isTimedTransition
      ? interpolate(timedTransitionProgress, [transitionFrames * 0.36, transitionFrames], [24, 0], {
          extrapolateLeft: 'clamp',
          extrapolateRight: 'clamp',
        })
      : 0
    : (() => {
        ...
        const slideFrame = loopFrame - loopActiveIndex * slideFrames;
        return orderedCues.length > 1 && slideFrame >= transitionStart
          ? interpolate(slideFrame - transitionStart, [0, transitionFrames * 0.42], [0, -28], {
              extrapolateLeft: 'clamp',
              extrapolateRight: 'clamp',
            })
          : 0;
  ...
  const secondaryHeadlineSize = timedMode ? previousHeadlineSize : nextHeadlineSize;
  const secondaryOpacity = timedMode
    ? isTimedTransition
      ? interpolate(timedTransitionProgress, [0, transitionFrames * 0.42], [1, 0], {
          extrapolateLeft: 'clamp',
          extrapolateRight: 'clamp',
        })
      : 0
    : (() => {
        ...
        const slideFrame = loopFrame - loopActiveIndex * slideFrames;
        return orderedCues.length > 1 && slideFrame >= transitionStart
          ? interpolate(slideFrame - transitionStart, [transitionFrames * 0.58, transitionFrames], [0, 1], {
              extrapolateLeft: 'clamp',
              extrapolateRight: 'clamp',
            })
          : 0;
  ...
  const secondaryY = timedMode
    ? isTimedTransition
      ? interpolate(timedTransitionProgress, [0, transitionFrames * 0.42], [0, -24], {
          extrapolateLeft: 'clamp',
          extrapolateRight: 'clamp',
        })
      : -24
    : (() => {
        ...
        const slideFrame = loopFrame - loopActiveIndex * slideFrames;
        return orderedCues.length > 1 && slideFrame >= transitionStart
          ? interpolate(slideFrame - transitionStart, [transitionFrames * 0.58, transitionFrames], [28, 0], {
              extrapolateLeft: 'clamp',
              extrapolateRight: 'clamp',
            })
          : 36;
      })();
  const headlineStyle = (fontSize: number, opacity: number, translateY: number): React.CSSProperties => ({
    position: 'absolute',
    left: 0,
    right: 0,
    top: '50%',
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h1 style={headlineStyle(currentHeadlineSize, currentOpacity, currentY)}>{currentHeadline}</h1>
          {orderedCues.length > 1 ? (
            <h1 style={headlineStyle(secondaryHeadlineSize, secondaryOpacity, secondaryY)}>{secondaryHeadline}</h1>
          ) : null}
        </div>
