...
export const activeVisual = (
  visuals: TimedVisual[],
  second: number,
): TimedVisual => {
  return (
    visuals.find((visual) => second >= visual.start && second < visual.end) ??
    visuals[0]
  );
};
...
export const visualMotionStyle = (
  visual: TimedVisual,
  progress: number,
): React.CSSProperties => {
...
export const VisualMediaLayer: React.FC<{
  ...
  if (visual.kind === "video") {
    const trimStart = Number(visual.trimStart ?? 0);
    const trimEnd = Number(visual.trimEnd);
    const startFrom = Number.isFinite(trimStart)
      ? Math.max(0, Math.round(trimStart * fps))
      : 0;
    const endAt = Number.isFinite(trimEnd)
      ? Math.max(startFrom + 1, Math.round(trimEnd * fps))
      : undefined;
    return (
      <OffthreadVideo
        muted={muted}
  ...
  return (
    <AbsoluteFill
      style={{
        ...visualMotionStyle(visual, progress),
