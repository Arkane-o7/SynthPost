...
Start at `stages.py` for the production lifecycle, `models.py` for data contracts, `config.py` for settings, `db/repository.py` for persistence, and `api/main.py` for app wiring. Feature packages own their logic. `narration/` owns dots.tts synthesis and sample-exact beat alignment; timeline and avatar stages consume its versioned canonical artifacts. Render boundaries consume approved state; they do not perform upstream editorial work.
