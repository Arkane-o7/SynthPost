...
| Narration / `narration_generate` | editorial | latest approved script, local dots.tts model and authorized voice profile | `narration_path`, `alignment_path` | canonical WAV and sample-exact beat/section clock |
...
Script approval queues both local dots.tts narration and visual discovery. They run concurrently on the editorial and media lanes because both read the immutable approved script while writing separate narration and visual artifacts. Other conflicting same-story jobs remain serialized, while work for other projects and episodes continues across the configured worker pool. The narration subprocess loads one MLX model and enrolls or loads one authorized voice profile, synthesizes each stable production beat, writes a continuous 48 kHz mono WAV, and records beat and section boundaries from the number of PCM samples actually written. It does not infer timing from word counts. The versioned artifacts live at `episodes/<episode_id>/stories/<story_id>/narration/script_vNNN/{narration.wav,alignment.json}` and are reused only when the script, model, voice/profile content, speed, language, sampler, beat text, and pause settings still match.
